...
/**
 * The print stylesheet hides everything outside `.print-area`, so this needs no separate
 * window or re-render - the browser's own dialog is the preview.
 */
...
<template>
  <div
    class="fixed inset-0 z-50 flex items-end justify-center bg-black/50 p-0 backdrop-blur-sm sm:items-center sm:p-4 print:bg-transparent print:backdrop-blur-none"
    role="dialog"
    aria-modal="true"
        ...
        <div>
          ...
          <ul v-if="recipe.ingredients.length" class="space-y-2">
            ...
            >
              ...
              <span>
                <strong v-if="ing.quantity">{{ ing.quantity }} {{ ing.unit }}</strong>
                {{ ing.foodItemName }}
              </span>
            </li>
          </ul>
